      ...
      <v-layout column class="mt-6 pl-3">
        ...
        <v-row >
          <v-layout column>
            ...
            <v-radio-group v-model="tool" class="mt-1" @change="updateTool">
        ...
        <v-row class="mt-7">
          <v-layout column>
            ...
            <v-radio-group v-model="item" class="mt-1" @change="updateTool">
            ...
            <v-slide-x-transition>
              <v-radio-group row v-if="item == 'cell'" label="Color" v-model="itemColor" class="mt-0" @change="update">
                ...
                <v-radio value=5 color="orange"></v-radio>
              </v-radio-group>
            </v-slide-x-transition>
            <v-slide-x-transition>
              <v-switch
...
export default {
  ...
  methods: {
    update() {
      ...
      delete data.item;
      delete data.itemColor;
      delete data.itemInfected;
      localStorage.setItem("tool."+this.tool+"."+this.item, JSON.stringify(data))

    },

    updateTool() {

      const options = JSON.parse(localStorage.getItem("tool."+this.tool+"."+this.item));
      if(options != null)
